...
function legacyCachedBinaryPath(pkg, platform, arch, env) {
  const target = binaryTarget(platform, arch);
  const configuredCache = brandedEnvValue(env, 'CACHE_DIR');
  const cacheRoot = configuredCache || join(env.XDG_CACHE_HOME || join(homedir(), '.cache'), 'bas-mcp-addon');
...
export async function findBinary(pkg, options = {}) {
  ...
  try { await stat(legacyPath); return legacyPath; } catch { return null; }
}
